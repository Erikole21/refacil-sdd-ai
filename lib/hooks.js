...
const os = require('os');
const { globalClaudeDir, globalCursorDir, globalOpenCodeDir, globalCodexDir } = require('./global-paths');
const { mergeCodexHooks, removeCodexHooks } = require('./toml-converter');
...
// ── Codex: hooks in ~/.codex/config.toml ────────────────────────────────────

// SDD hooks to inject into Codex config.toml
const CODEX_SDD_HOOKS = [
  ...
  { marker: '_sdd_review', event: 'preToolUse', command: 'refacil-sdd-ai check-review', matcher: 'Bash' },
  { marker: '_sdd_notify', event: 'userPromptSubmit', command: 'refacil-sdd-ai notify-update' },
];

/**
 * Install SDD hooks into the global ~/.codex/config.toml.
 * Creates the file if absent. Merges without overwriting non-SDD keys.
 * @param {string} [homeDir] - user home directory (injectable; default: os.homedir())
 * @returns {boolean} true if file was written, false if already up to date
 */
function installCodexHooks(homeDir) {
  const resolvedHome = homeDir || os.homedir();
  ...
  if (fs.existsSync(configPath)) {
    try { existingToml = fs.readFileSync(configPath, 'utf8'); } catch (_) { existingToml = ''; }
  }

  const merged = mergeCodexHooks(existingToml, CODEX_SDD_HOOKS);
  if (merged === null) {
    // Invalid TOML — do not overwrite
    process.stderr.write('[refacil-sdd-ai] Codex hooks: could not merge hooks into invalid config.toml.\n');
    return false;
  }

  // Check idempotence: if merged is identical to existing, nothing changed
  if (merged === existingToml) {
    console.log('  Hooks SDD-AI ya configurados en ~/.codex/config.toml.');
    return false;
  }
  ...
  try {
    ...
    process.stderr.write(`[refacil-sdd-ai] Could not write ~/.codex/config.toml: ${err.message}\n`);
    return false;
  }
}

/**
 * Uninstall SDD hooks from the global ~/.codex/config.toml.
 * Removes only entries with _sdd* markers. Deletes the file if it becomes empty/meaningless.
 * @param {string} [homeDir] - user home directory (injectable; default: os.homedir())
 * @returns {boolean} true if changes were made
 */
function uninstallCodexHooks(homeDir) {
  const resolvedHome = homeDir || os.homedir();
  const codexDir = globalCodexDir(resolvedHome);
  const configPath = path.join(codexDir, 'config.toml');
  ...
  let existingToml;
  try { existingToml = fs.readFileSync(configPath, 'utf8'); } catch (_) { return false; }

  const cleaned = removeCodexHooks(existingToml);
  if (cleaned === null) {
    process.stderr.write('[refacil-sdd-ai] Codex hooks: could not parse config.toml for removal.\n');
    return false;
  }

  if (cleaned === existingToml) return false;

  // If the resulting TOML has no meaningful content, delete the file
  if (!cleaned.trim()) {
...
function installHooks(ideDir, homeDir, projectRoot) {
  ...
  if (ideDir === '.codex') return installCodexHooks(homeDir);
...
function uninstallHooks(ideDir, homeDir, projectRoot) {
  ...
  if (ideDir === '.codex') return uninstallCodexHooks(homeDir);
